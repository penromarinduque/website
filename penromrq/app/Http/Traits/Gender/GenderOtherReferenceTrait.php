...
trait GenderOtherReferenceTrait	
{

	public function gender_other_reference_rules()
	{
		return [
	...
	public function gender_create_other_reference($method, $id = null, $request)
	{

		$rules = $this->gender_other_reference_rules();
		...
		Session::flash('success','New Reference successfully created.');
		return back();
	
	}

	public function gender_edit_other_reference($method, $id = null, $request)
	{
		...
		if(count($PanelDetails) > 0) {
		    ...
		    Session::flash('failed','Reference data do not exists.');
		    return back();
		}
	
	}

	public function gender_update_other_reference($method, $id = null, $request)
	{
	...
	public function gender_toggle_other_reference_tab($method, $id = null, $request)
	{
	...
	public function gender_toggle_other_reference($method, $id = null, $request) 
	{
		$panelDetails = app('GenderPanelDetails')->where('detail_id', Crypt::decrypt($id));
		if(count($panelDetails->first()) > 0) {
			$panelDetailsContent = $this->gender_panel_detail_content($panelDetails->first());
	...
	public function gender_delete_other_reference($method, $id = null, $request)
	{
		...
		Session::flash('success','Reference successfully deleted.');
		return back();
