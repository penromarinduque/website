...
trait WebsiteLayoutTrait
{
    ...
    public function center_panel_details_storage_images()
    {
        return app('PanelDetailsStorage')
                    ->select('storage_id','file_path')
                    ->where('file_type','I')
                    ->where('status','1')
    ...
    public function website_count_center_bar_details($center_id)
    {
        return app('CenterBarDetails')->select('detail_id')->where('center_id',$center_id)->where('status','1')->count();
    }
