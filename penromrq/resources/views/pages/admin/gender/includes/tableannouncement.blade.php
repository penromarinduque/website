    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center" style="vertical-align: middle;">
                    <a href="{{ route('gender.route',['path' => $path,'action' => 'gender-edit-announcement','id' => Crypt::encrypt($value->detail_id)]) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                    <a href="{{ route('gender.route',['path' => $path,'action' => 'gender-delete-announcement','id' => Crypt::encrypt($value->detail_id)]) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this row?')"><i class="fa fa-trash"></i></a>
                </td>
            </tr>
            @endforeach
            <tr>
                ...
                @if(count($webdata->gender_retrieve_all_panel_details($GenderPanel->panel_id)) == 0)
                    No result's found!, Try to <a href="#newpost" data-toggle="tab">Add New Announcement</a>.
                @else
